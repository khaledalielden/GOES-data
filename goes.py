...
class goes_xray:
    def __init__(self, start_time, end_time, SatelliteNumber):
        ...
        sdate, stime = start_time.split(' ')
        year, month, day= sdate.split('-')
        hour, minute = stime.split(':')
    
        edate, etime = end_time.split(' ')
        year_e, month_e, day_e= edate.split('-')
        hour_e, minute_e = etime.split(':')
    ...
    def get_file(self,date_time_):
        date = str(date_time_).split(' ')[0]
    
        if ((self.SatelliteNumber == 13) or (self.SatelliteNumber == 14) or (self.SatelliteNumber == 15)):
            year, month, day= date.split('-')
            
            call_path = 'https://www.ncei.noaa.gov/data/goes-space-environment-monitor/access/science/xrs/goes'+str(self.SatelliteNumber)+'/gxrs-l2-irrad_science/'+str(year)+'/'+str("{0:02d}".format(int(month)))+'/'
          
            filename = 'sci_gxrs-l2-irrad_g'+str(self.SatelliteNumber ) +'_d'+str(year)+str("{0:02d}".format(int(month)))+str("{0:02d}".format(int(day)))+'_v0-0-0.nc'

        else:
            year, month, day= date.split('-')
       
            call_path = 'https://data.ngdc.noaa.gov/platforms/solar-space-observing-satellites/goes/goes'+str(self.SatelliteNumber )+'/l2/data/xrsf-l2-flx1s_science/'+str(year)+'/'+str("{0:02d}".format(int(month)))+'/'
    
            filename = 'sci_xrsf-l2-flx1s_g'+str(self.SatelliteNumber) +'_d'+str(year)+str("{0:02d}".format(int(month)))+str("{0:02d}".format(int(day)))+'_v2-2-0.nc'
    ...
    def get_data(self, path=None, save_files = False):
        ...
        for f in range(int(files_number)):
            ...
            try:
                ...
                raise FileNotFoundError("data are not available. Try another satellite")
